...
박람회 현장 대형 TV 송출용 실시간 이벤트 순위판 + 휴대폰용 관리자 입력 페이지.

- **TV 송출 화면** (`/`) : 장거리 / 노래방 1위, 상품 안내, 안내 멘트 자동 전환, NEW RECORD 효과
- **관리자 화면** (`/admin`) : 비밀번호 입력 후 기록 등록 / 1위 확인 / 초기화
...
- `karaoke_records   (id, name, phone_tail, score,    created_at)`
...
### TV 송출 화면 (`/`)
- 검정 + 골드 컬러 / 멀리서도 보이도록 1위 숫자 매우 크게
